...
const SocketProvider = ({ children }: { children: ReactNode }) => {
    ...
    const handleRequestDrawing = useCallback(
        ({ socketId }: { socketId: SocketId }) => {
            socket.emit(SocketEvent.SYNC_SKETCH, { socketId, drawingData })
        },
        [drawingData, socket],
    )
    ...
    useEffect(() => {
        ...
        socket.on(SocketEvent.USERNAME_ALREADY_TAKEN, handleUsernameExist)
        socket.on(SocketEvent.ACCEPT_JOIN, handleJoiningAccept)
        socket.on(SocketEvent.USER_LEFT, handleUserLeft)
        socket.on(SocketEvent.INIT_DRAWING, handleRequestDrawing)
        socket.on(SocketEvent.SYNC_SKETCH, handleDrawingSync)

        return () => {
            socket.off("connect_error")
            socket.off("connect_failed")
            socket.off(SocketEvent.USERNAME_ALREADY_TAKEN)
            socket.off(SocketEvent.ACCEPT_JOIN)
            socket.off(SocketEvent.USER_LEFT)
            socket.off(SocketEvent.INIT_DRAWING)
            socket.off(SocketEvent.SYNC_SKETCH)
        }
    }, [
        handleDrawingSync,
        handleError,
