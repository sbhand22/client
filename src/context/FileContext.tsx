...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const createDirectory = useCallback(
        (
            ...
            socket.emit(SocketEvent.CREATE_FOLDER, {
                parentDirId,
                newDirectory,
            })
    ...
    const updateDirectory = useCallback(
        (
            ...
            socket.emit(SocketEvent.MODIFY_FOLDER, {
                dirId,
                children,
            })
        },
    ...
    const renameDirectory = useCallback(
        (
            ...
            socket.emit(SocketEvent.RENAME_FOLDER, {
                dirId,
                newDirName,
            })
    ...
    const deleteDirectory = useCallback(
        (dirId: string, sendToSocket: boolean = true) => {
            ...
            if (!sendToSocket) return
            socket.emit(SocketEvent.DELETE_FOLDER, { dirId })
        },
        [socket],
    )
    ...
    const createFile = useCallback(
        (
            ...
            socket.emit(SocketEvent.CREATE_FILE, {
                parentDirId,
                newFile,
            })
    ...
    const renameFile = useCallback(
        (
            ...
            if (!sendToSocket) return true
            socket.emit(SocketEvent.RENAME_FILE, {
                fileId,
                newName,
            })
    ...
    const deleteFile = useCallback(
        (fileId: string, sendToSocket: boolean = true) => {
            ...
            if (!sendToSocket) return
            socket.emit(SocketEvent.REMOVE_FILE, { fileId })
        },
    ...
    const handleUserJoined = useCallback(
        ({ user }: { user: RemoteUser }) => {
            ...
            socket.emit(SocketEvent.UPDATE_FILE_SYSTEM, {
                fileStructure,
                openFiles,
                activeFile,
            ...
            socket.emit(SocketEvent.SYNC_SKETCH, {
                drawingData,
                socketId: user.socketId,
            })
    ...
    useEffect(() => {
        socket.once(SocketEvent.UPDATE_FILE_SYSTEM, handleFileStructureSync)
        socket.on(SocketEvent.NEW_USER_CONNECTED, handleUserJoined)
        socket.on(SocketEvent.CREATE_FOLDER, handleDirCreated)
        socket.on(SocketEvent.MODIFY_FOLDER, handleDirUpdated)
        socket.on(SocketEvent.RENAME_FOLDER, handleDirRenamed)
        socket.on(SocketEvent.DELETE_FOLDER, handleDirDeleted)
        socket.on(SocketEvent.CREATE_FILE, handleFileCreated)
        socket.on(SocketEvent.UPDATE_FILE, handleFileUpdated)
        socket.on(SocketEvent.RENAME_FILE, handleFileRenamed)
        socket.on(SocketEvent.REMOVE_FILE, handleFileDeleted)

        return () => {
            socket.off(SocketEvent.NEW_USER_CONNECTED)
            socket.off(SocketEvent.CREATE_FOLDER)
            socket.off(SocketEvent.MODIFY_FOLDER)
            socket.off(SocketEvent.RENAME_FOLDER)
            socket.off(SocketEvent.DELETE_FOLDER)
            socket.off(SocketEvent.CREATE_FILE)
            socket.off(SocketEvent.UPDATE_FILE)
            socket.off(SocketEvent.RENAME_FILE)
            socket.off(SocketEvent.REMOVE_FILE)
        }
    }, [
        handleDirCreated,
        handleDirDeleted,
